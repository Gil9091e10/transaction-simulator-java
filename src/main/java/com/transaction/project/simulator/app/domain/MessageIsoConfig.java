...
public class MessageIsoConfig implements Serializable {
    ...
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "messageIsoConfig")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    ...
    public void setMessageFieldsConfigs(Set<MessageFieldsConfig> messageFieldsConfigs) {
        if (this.messageFieldsConfigs != null) {
            this.messageFieldsConfigs.forEach(i -> i.setMessageIsoConfig(null));
        }
        if (messageFieldsConfigs != null) {
            messageFieldsConfigs.forEach(i -> i.setMessageIsoConfig(this));
        }
        this.messageFieldsConfigs = messageFieldsConfigs;
    }

    public MessageIsoConfig messageFieldsConfigs(Set<MessageFieldsConfig> messageFieldsConfigs) {
        this.setMessageFieldsConfigs(messageFieldsConfigs);
        return this;
    }

    public MessageIsoConfig addMessageFieldsConfig(MessageFieldsConfig messageFieldsConfig) {
        this.messageFieldsConfigs.add(messageFieldsConfig);
        messageFieldsConfig.setMessageIsoConfig(this);
        return this;
    }

    public MessageIsoConfig removeMessageFieldsConfig(MessageFieldsConfig messageFieldsConfig) {
        this.messageFieldsConfigs.remove(messageFieldsConfig);
        messageFieldsConfig.setMessageIsoConfig(null);
        return this;
    }

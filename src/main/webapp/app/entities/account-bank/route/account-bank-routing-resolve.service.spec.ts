...
import accountBankResolve from './account-bank-routing-resolve.service';

describe('AccountBank routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: AccountBankService;
  let resultAccountBank: IAccountBank | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IAccountBank returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        accountBankResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultAccountBank).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IAccountBank>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };

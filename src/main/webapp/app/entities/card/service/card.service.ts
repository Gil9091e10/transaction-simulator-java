...
type RestOf<T extends ICard | NewCard> = Omit<T, 'expirationDate'> & {
...
export class CardService {
  ...
  protected convertDateFromClient<T extends ICard | NewCard | PartialUpdateCard>(card: T): RestOf<T> {
    return {
      ...card,
      expirationDate: card.expirationDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCard: RestCard): ICard {
    return {
      ...restCard,
      expirationDate: restCard.expirationDate ? dayjs(restCard.expirationDate) : undefined,
    };
  }

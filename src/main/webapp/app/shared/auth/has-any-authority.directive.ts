 ...
 * @howToUse
 * ```
 *     <some-element *stpHasAnyAuthority="'ROLE_ADMIN'">...</some-element>
 *
 *     <some-element *stpHasAnyAuthority="['ROLE_ADMIN', 'ROLE_USER']">...</some-element>
 * ```
 */
@Directive({
  standalone: true,
  selector: '[stpHasAnyAuthority]',
})
export default class HasAnyAuthorityDirective {
  public authorities = input<string | string[]>([], { alias: 'stpHasAnyAuthority' });
